...
if args.multi_material:
    ...
    if args.shape_normalize:
        dataset = NITO_Dataset(topologies, [BCs, loads], [vfs, shapes/shapes.max(1,keepdims=True)], shapes, n_samples=10, consistent_batch=consistent_batch)
    else:
        dataset = NITO_Dataset(topologies, [BCs, loads], [vfs, shapes], shapes, n_samples=10, consistent_batch=consistent_batch)
    BC_shape = [dim*2, dim*2]
    C_shape = [1, dim]
