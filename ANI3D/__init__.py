...
def animate_cross_voxelized(top, shape, bc=None, load=None, n_frames = 50, fps=25, f_name='top_anim.mp4', grid=False, face_color='white', edge_color='#87CEFA', highlight_color='royalblue', meat_color='indigo'):
    ...
    def get_frame(i):
        ...
        if bc is not None:
            plotter.add_mesh(x_mesh, color='red', point_size=10, render_points_as_spheres=True, opacity=0.7)
            plotter.add_mesh(y_mesh, color='green', point_size=10, render_points_as_spheres=True, opacity=0.7)
            plotter.add_mesh(z_mesh, color='blue', point_size=10, render_points_as_spheres=True, opacity=0.7)
...
def animate_top_series(tops, shape, bc=None, load=None, fps=25, f_name='top_anim.mp4', grid=False, face_color='lightgrey', edge_color=None):
    f_name_t = uuid.uuid4().hex
